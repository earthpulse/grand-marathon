<script>
    let { isOpen, action, onClose } = $props();
    ...
    let sortedTrace = $derived(
        action && action.trace
            ? [...action.trace].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
            : []
    );
</script>
            ...
            <header class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">
                        Track execution milestones, status updates, and chronological changes of action: <span class="font-semibold text-gray-700">{action.title}</span>
                    </p>
                </div>
                <button
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-6">
                {#if sortedTrace.length === 0}
                    <div class="text-center py-8 text-gray-500 text-sm">
                        No trace entries found for this action.
                    </div>
                {:else}
